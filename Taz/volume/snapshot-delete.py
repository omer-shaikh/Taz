...
def delete_qa(client, qa_table):
    ...
    row=db.selectNonTerminatedResources(qa_table, '0', notification_date.strftime('%d-%m-%Y'), 'Yes')
    print(row)
    for snapshot in row:
...
def delete_prod(client, prod_table):
    ...
    notification_date = d + timedelta(days=-1)
    print (notification_date.strftime('%d-%m-%Y'))
    row=db.selectNonTerminatedResources(prod_table, '0', notification_date.strftime('%d-%m-%Y'), 'Yes')
    print(row)
    for snapshot in row:
        print('Terminate this snapshot=', snapshot.get("SnapshotId"))
        client.delete_snapshot(SnapshotId=snapshot.get("SnapshotId"))
...
def delete_stagging(client, stagging_table):
    ...
    notification_date = d + timedelta(days=-1)
    print (notification_date.strftime('%d-%m-%Y'))
    row=db.selectNonTerminatedResources(stagging_table, '0', notification_date.strftime('%d-%m-%Y'), 'Yes')
    print(row)
    for snapshot in row:
        print('Terminate this snapshot=', snapshot.get("SnapshotId"))
        client.delete_snapshot(SnapshotId=snapshot.get("SnapshotId"))
