...
def amiCleanupOperations_QA(botoClient, msg, qa_table):
    ...
    for r in response['Images']:
        ...
        if do_not_delete_ami:
            ...
            print ('Images are important ' +  r['ImageId'])
        elif utils.active(r['Tags']):
            imp_ami.append(r['ImageId'])
            print ('Images are important ' +  r['ImageId'])
        else:
            if older_than_1_day :
                    ...
                    if utils.active(r['Tags']):
                        ...
                        if always_true:
                            if older_than_10_day:
                                ...
                                db.putResourceInfoToDatabase(qa_table,r['ImageId'], r['CreationDate'], '0', d.strftime('%d-%m-%Y'),'Yes',"enter comment if required")
                            else:
                    ...
                    if utils.active(r['Tags']):
                        ...
                        if always_true:
                            ...
                            if older_than_15_day:
                                ...
                                delete_ami[r['ImageId']]=r['Name']
                                db.putResourceInfoToDatabase(prod_table,r['ImageId'], r['CreationDate'], '0', d.strftime('%d-%m-%Y'),'Yes',"enter comment if required")
                            else:
                    ...
                    if utils.active(r['Tags']):
                        ...
                        if always_true:
                            if older_than_10_day:
                                print('10 days are done please delete ' + r['Name'])
                                delete_ami[r['ImageId']]=r['Name']
                                db.putResourceInfoToDatabase(stagging_table,r['ImageId'], r['CreationDate'], '0', d.strftime('%d-%m-%Y'),'Yes',"enter comment if required")
                            else:
                                print('it has always true tag but 10 days are not done leave')
                        elif only_true:
                            if older_than_3_day:
